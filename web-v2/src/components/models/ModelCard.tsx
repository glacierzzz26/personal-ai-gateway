...
export function bestPrice(m: ModelCatalogItem): { inP: number; outP: number } | null {
  ...
  const inP = Math.min(...on.map(o => o.inputPriceUsd));
  const match = on.find(o => o.inputPriceUsd === inP)!;
  return { inP, outP: match.outputPriceUsd };
}
...
export default function ModelCard({ model, picked, onOpen, onToggleCompare, onToggleEnabled }: Props) {
      ...
      <div>
        {p ? (
          <span className="gw-num" style={{ fontSize: 15, fontWeight: 500, color: 'var(--gw-primary)' }}>
            {fmt.price(p.inP)} / {fmt.price(p.outP)}
          </span>
        ) : (
          <span style={{ color: 'var(--gw-text-3)', fontSize: 13 }}>未配置供给源</span>
        )}
        <span style={{ fontSize: 11, color: 'var(--gw-text-3)', marginLeft: 4 }}>每 1M tokens</span>
      </div>
