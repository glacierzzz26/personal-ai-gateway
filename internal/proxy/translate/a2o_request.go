...
func buildA2ORequest(body []byte, stream bool) (outOp string, outBody []byte, estIn int, err error) {
	...
	for _, m := range in.Messages {
		msgs = append(msgs, rewriteMessage(m)...)
	}
	if sys != "" {
...
func rewriteMessage(m aMessage) []map[string]any {
	...
	case "assistant":
		if o := assistantToOpenAI(m.Content); o != nil {
...
func assistantToOpenAI(raw json.RawMessage) map[string]any {
	if s, ok := asTextString(raw); ok {
