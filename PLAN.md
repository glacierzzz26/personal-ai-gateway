...
- **无成本依据的供给源不能启用**(issue #26):成本 = 官方价 × ratio,若官方价拿不到(未绑定 / 官方价单价为 0 /
  汇率缺失)且手填四价全为 0,则这条供给源**没有任何成本数字**,启用它等于免费放流量(账面毛利假的 100%)。
  故在**启用那一步**拦下(新建 / 改价 / 模型启用联动三处,服务端为准),前端置灰并说明差什么。只挡状态迁移,
  不自动停用存量。判据单一真源 = `pricing.ZeroPriced`(见 DESIGN §5.8)。
- **换汇**:官方价是 USD;算售价时过 `settings.USDPerCNY`(已有)。汇率为 0 = 未设 → **拒绝计算并提示**,不臆造(与 `convertPrice` 同规矩)。

### 经营口径(运行总览首屏,issue #13)

首屏以「站主的一天」为中心重排,金额只算**客户归属流量**(`role=user` 名下的请求):

| 数 | 来源 | 说明 |
|---|---|---|
| **营收** | `SUM(charge_usd)` | 客户实付你 |
| **成本** | `SUM(cost)` | 你付上游(仅客户流量,站主自用不计) |
| **毛利 / 毛利率** | 两者之差 / ÷营收 | 营收为 0 时毛利率记 0,不臆造 100% |

- **为什么不按 `charge_usd>0` 排除旧日志**(改造前无售价):那批日志 `owner_id=0`,按角色过滤天然排除;而 `charge_usd` 本可合法为 0(倍率 0、零 token 的失败请求),拿它当「有效样本」会误杀真实流量。故用 `EXISTS(admins.role='user')` 判定(见 `store.customerCond`)。
- **默认窗口 = 最近 1 天**,环比基准是**昨日整日**(自然日对齐),不是前 1×24 小时 —— 避免跨零点漂移。
- **欠费线**:余额 ≤0 = 已欠费(转发被拒,客户在流失);0 < 余额 < 近 1 天消耗 = 余额预警。
- 营收/成本/毛利**仅 admin 可见**:`/overview` 与 `/customers/focus` 均走 `requireAdmin`,user 侧 403(与既有 `/overview` 同规矩)。
...
## 7.1 域名上线(2026-09-21,已实施)

备案通过后 `5home.online` 上线,公网入口从「裸 IP + 非标端口 + 自签证书」改为「域名 + 公信证书 + 宿主 Nginx」。
**两面各占一个子域、都在 443**:`gateway.5home.online` = 管理台/登录面,`gatewayapi.5home.online` = 数据面(API)。
主域 `5home.online` 因此空出来,留给别的用途。

| 阶段 | 内容 | 验收 |
|---|---|---|
| **D0** ✓ | 宿主发布收窄 `127.0.0.1:17081:17080`(数据面)/ `127.0.0.1:17090:17090`(管理台) | 容器内监听口不变、无需重建镜像;发布口与 Nginx 监听口必须错开(同号撞 bind) |
| **D1** ✓ | 宿主 Nginx 边缘(**独立仓库 `host-infra`**,宿主级多服务边缘):443 按 SNI 主机名分两面(管理台/数据面);`default_server` 返 444 | `nginx -t` 通过;两子域 443 均可用;裸 IP/未知主机名 443 被 444 拒 |
| **D2** ✓ | DNS-01 签通配符 `*.5home.online`(DNSPod),自动续期 | 浏览器开 `https://gateway.5home.online` 登录管理台**无需导 CA** |
| **D3** | 客户端切换 + `public_base_url` 设为 `https://gatewayapi.5home.online` | 用令牌打数据面子域 `/v1/*` 成功、SSE 首字节不被缓冲;面隔离(错面 404)保持 |
| **D4** ✓ | **收敛入口**:Nginx 下线 17080 过渡口(不再 `listen 17080`),容器发布口全部只绑 `127.0.0.1` | 只有两个域名可达;`IP:17080`/`IP:17090`/`5home.online:17080` 从公网全部不可达;两域名功能不变 |

> 落地细节与回滚见 `deploy/README.md`「域名边缘」与 `deploy/DR.md` §1.1。
