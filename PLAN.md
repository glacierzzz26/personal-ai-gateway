...
| 阶段 | 内容 | 验收 |
|---|---|---|
| **D0** ✓ | 数据面宿主发布收窄 `127.0.0.1:17080:17080`(公网 17080 归 Nginx);管理台 17090 不动 | 容器内监听口不变、无需重建镜像;回滚 = 该行改回 `17080:17080` |
| **D1** ✓ | 宿主 Nginx 边缘(`deploy/nginx/ai-gateway-edge.conf` + `deploy/scripts/setup-edge.sh`):443→管理台、17080 SNI 双证书(域名公信 / 裸 IP 自签回退) | `nginx -t` 通过;旧 `IP:17080`(自签)与新 `域名:17080`(公信)并存可用 |
| **D2** ✓ | DNS-01 签通配符 `*.5home.online`(DNSPod),自动续期 | 浏览器开 `https://5home.online` 登录管理台**无需导 CA** |
| **D3** | 客户端切换 + `public_base_url` 设为 `https://5home.online:17080` | 用令牌打域名 `/v1/*` 成功、SSE 首字节不被缓冲;面隔离(错面 404)保持 |
