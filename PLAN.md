...
| 阶段 | 内容 | 验收 |
|---|---|---|
| **D0** ✓ | 数据面宿主发布收窄 `127.0.0.1:17081:17080`(公网 17080 归 Nginx,同号会撞 bind);管理台 17090 不动 | 容器内监听口不变、无需重建镜像;回滚 = 该行改回 `17080:17080`(须先停 Nginx) |
| **D1** ✓ | 宿主 Nginx 边缘(**独立仓库 `host-infra`**,宿主级多服务边缘):443 按 SNI 主机名分两面(管理台/数据面),另留 17080 为数据面**过渡口**(SNI 双证书:域名公信 / 裸 IP 自签回退) | `nginx -t` 通过;两子域 443 均可用;旧 `IP:17080`(自签)与新域名(公信)并存 |
