...
| 供给源(offer) | 某模型挂在某渠道上的报价(输入/输出/缓存读/缓存写价,$/1M tokens)、RPM、启用与否;顺序 = 请求尝试次序。缓存写价留 0 = 无依据,该部分 token 按输入价计(见 DESIGN §5.7)。**无成本依据的供给源不能启用**:官方价拿不到(未绑定 / 官方价单价为 0 / 汇率缺失)且手填四价全为 0 → 启用被拒(`400 zero_price`),因为开了等于免费放流量;模型启用时的供给源联动也会跳过这类供给源并在响应里回报,前端把开关置灰并说明差什么(见 DESIGN §5.8)。 |
