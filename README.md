...
- 页面覆盖:运行总览(近 7 天)/ 渠道管理(CRUD + 连通测试 + 同步模型)/ 模型广场(供给源管理、定价、拖拽排序、设为首选、用量)/ 路由规则 / 访问令牌 / 请求日志(服务端分页/筛选/详情)/ 用量统计(model|channel|token × 7|30 天)/ 系统设置(超时、重试、自动降级、代理、TLS、日志保留/采样、时区、清空日志)。
